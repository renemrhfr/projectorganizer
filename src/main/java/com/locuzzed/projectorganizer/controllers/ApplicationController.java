...
import com.locuzzed.projectorganizer.types.Sendout;
import com.locuzzed.projectorganizer.types.Settings;
import com.locuzzed.projectorganizer.types.Track;
import com.locuzzed.projectorganizer.types.TrackVersion;
...
public class ApplicationController {
    ...
    public void addTrackToReleaseSchedule(Track track) {
        if (track.isDone() && !ResultFilter.includeDone) {
            return;
        }
        boolean textCheck = ResultFilter.textFilter.equals("") || track.getName().contains(ResultFilter.textFilter);
        if(!textCheck) {
            return;
        }
        // First Row
        ...
        if (track.getProjectFolder() != null && !Objects.equals(track.getProjectFolder(), "")) {
            projectsButton.setOnAction(e -> {
                try {
                    Runtime.getRuntime()
                            .exec("explorer " + DropboxPathHelper.convertPath(track.getProjectFolder()));
        ...
        if (track.getPreviewFolder() != null && !Objects.equals(track.getPreviewFolder(), "")) {
            previewsButton.setOnAction(e -> {
                try {
                    Runtime.getRuntime().exec("explorer " + DropboxPathHelper.convertPath(track.getPreviewFolder()));
                } catch (IOException ex) {
                    throw new RuntimeException(ex);
                }
            });
    ...
    public void addTrackToWIP(Track track) {
        boolean tag1Check = !ResultFilter.tag1 || track.getTags().contains(dataStore.getTags().get(0));
        boolean tag2Check = !ResultFilter.tag2 || track.getTags().contains(dataStore.getTags().get(1));
        boolean tag3Check = !ResultFilter.tag3 || track.getTags().contains(dataStore.getTags().get(2));
        boolean tag4Check = !ResultFilter.tag4 || track.getTags().contains(dataStore.getTags().get(3));
        boolean tag5Check = !ResultFilter.tag5 || track.getTags().contains(dataStore.getTags().get(4));
        boolean textCheck = ResultFilter.textFilter.equals("") || track.getName().contains(ResultFilter.textFilter);
        ...
        if (track.getPreviewFolder() != null && !Objects.equals(track.getPreviewFolder(), "")) {
            previewsButton.setOnAction(e -> {
                try {
                    Runtime.getRuntime()
                            .exec("explorer " + DropboxPathHelper.convertPath(track.getPreviewFolder()));
                } catch (IOException ex) {
                    throw new RuntimeException(ex);
                }
            });
    ...
    public void refreshFilter() {
        ResultFilter.includeDone = filterDone.isSelected();
        ResultFilter.tag1 = filterTag01.isSelected();
        ResultFilter.tag2 = filterTag02.isSelected();
        ResultFilter.tag3 = filterTag03.isSelected();
        ResultFilter.tag4 = filterTag04.isSelected();
        ResultFilter.tag5 = filterTag05.isSelected();
        ResultFilter.textFilter = filterName.getText();
        refreshUI();
    }
}